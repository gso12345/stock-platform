...
/**
 * "전체" 탭에서는 자산 추가 버튼이 보이면 안 된다는 규칙을 고정한다.
 *
 * (버튼 이름이 "종목 추가" 였다. 현금 담기와 한 창으로 합치면서
 *  "자산 추가" 가 됐다 — 담는 것이 종목만이 아니게 됐다.)
 *
...
describe("내 자산 — 자산 추가 버튼 노출 규칙", () => {
  ...
  it("기본으로 열리는 '전체' 탭에서는 자산 추가 버튼이 없다", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByRole("button", { name: /자산 추가/ })).not.toBeInTheDocument();
    });
  });
  ...
  it("개별 포트폴리오를 고르면 자산 추가 버튼이 나타난다", async () => {
