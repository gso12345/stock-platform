...
describe("용어 사전 내용", () => {
  ...
  it("'얼마면 좋다'를 말하지 않는다", () => {
    /* 처음에는 'PER 10배 아래면 싼 편' 처럼 기준을 같이 줬다. 업종마다
       기업마다 보통 수준이 달라 그대로 대면 틀린다 — 경기민감 업종은
       실적이 정점일 때 PER 이 가장 낮게 나오고, 은행은 부채비율이
       1,000%를 넘는 것이 정상이다. 그래서 뜻만 남겼다. */
    const 단정 = /[0-9]+\s*(배|%)\s*(아래|위|이상|이하)면\s*(싸|싼|비싸|비싼|좋|안정|넉넉|많)/;
    const 걸린것 = Object.entries(용어사전)
      .filter(([, v]) => 단정.test(v.뜻))
      .map(([k, v]) => `${k}: ${v.뜻}`);
    expect(걸린것).toEqual([]);
  });

  it("뜻 말고 다른 칸을 두지 않는다", () => {
    /* 기준 칸을 남겨두면 언젠가 누가 다시 채운다. 아예 없앤다 */
    const 칸 = new Set(Object.values(용어사전).flatMap((v) => Object.keys(v)));
    expect([...칸].sort()).toEqual(["뜻", "이름"]);
  });
