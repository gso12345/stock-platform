...
describe("끊겨 있던 길을 잇는다", () => {
  ...
  it("보유 조회가 새 요청을 만들지 않는다", () => {
    /* 내 자산·퀀트·글쓰기가 이미 받아 둔 것을 읽는다.
       예전에는 이름표를 여기 직접 적었다. 그런데 일곱 곳이 같은
       이름표를 각자 정의하고 있었고, **먼저 붙은 쪽의 fetcher 가
       이겨서** 내 자산의 고침이 통째로 죽어 있었다. 지금은 정의가
       한 벌이다(hooks/usePortfolioItems). 요청이 안 느는 것은
       그대로이고, 이제 시세까지 같이 받아 온다. */
    expect(코드).toMatch(/use보유목록\(/);
  });
