...
describe("기준일은 온전한 날에서 고른다", () => {
  /**
   * 서버 기록에는 **시세를 일부만 받은 날**이 섞일 수 있다.
   * 못 받은 종목을 매입가로 세므로 그날 값이 실제보다 낮다(실측 25%).
   *
   * 그 날이 기준일이 되면 **그 뒤 모든 퍼센트가 부풀려진다.** 그리고
   * 서버가 그 줄을 나중에 고쳐 쓰면 같은 날의 수익률이 볼 때마다
   * 달라진다 — '앱을 안 연 날 수익률이 자꾸 다르게 나온다' 가 이것이다.
   *
   * priced 는 '시세가 있어야 하는 종목 수', filled 는 '실제로 받은 수'.
   * 둘이 같아야 온전하다.
   */
  const 온전 = (day: string, value: number) =>
    ({ day, value, cost: 900_000, filled: 2, priced: 2 });
  const 반쪽 = (day: string, value: number) =>
    ({ day, value, cost: 900_000, filled: 1, priced: 2 });

  it("첫날이 반쪽이면 그 다음 온전한 날을 기준으로 삼는다", () => {
    const r = 견주기([
      반쪽("2026-08-20", 750_000),      // 시세를 하나만 받은 날 — 25% 낮다
      온전("2026-08-21", 1_000_000),
      온전("2026-08-24", 1_100_000),
    ]);
    expect(r.기준칸).toBe(1);
    /* 기준이 반쪽 날이었다면 +46.7% 로 부풀려진다 */
    expect(r.점들[2].내수익).toBeCloseTo(10, 1);
  });

  it("반쪽 날이 섞여 있어도 나머지 퍼센트는 그대로다", () => {
    const r = 견주기([
      온전("2026-08-20", 1_000_000),
      반쪽("2026-08-21", 750_000),
      온전("2026-08-24", 1_100_000),
    ]);
    expect(r.기준칸).toBe(0);
    expect(r.점들[2].내수익).toBeCloseTo(10, 1);
  ...
  it("옛 기록(칸이 비어 있음)은 예전처럼 다룬다", () => {
    /* priced·filled 가 생기기 전에 쌓인 줄은 둘 다 0 이다.
       그걸 '반쪽' 으로 보면 옛 기록이 통째로 기준에서 빠진다 —
       있는 정보로만 판단한다 */
    const r = 견주기([
      { day: "2026-08-20", value: 1_000_000, cost: 900_000, filled: 0, priced: 0 },
      { day: "2026-08-24", value: 1_100_000, cost: 900_000, filled: 0, priced: 0 },
    ]);
    expect(r.기준칸).toBe(0);
    expect(r.점들[1].내수익).toBeCloseTo(10, 1);
  });

  it("옛 기록이 앞에 있어도 그것을 기준으로 삼는다", () => {
    /* 위 검사만으로는 부족하다 — 옛 기록만 있으면 폴백이 어차피
       같은 답을 준다. **옛 기록 뒤에 온전한 날이 오는** 경우라야
       '옛 기록을 반쪽으로 보는지' 가 갈린다.
       (그 구분이 없어서 뮤테이션이 살아남았다.)

       priced·filled 칸이 생기기 전 기록을 반쪽으로 보면, 그 시절
       자산 흐름이 통째로 기준에서 빠져 수익률이 짧은 구간만으로
       잡힌다 — 오래 쓴 사람일수록 크게 틀어진다. */
    const r = 견주기([
      { day: "2026-08-20", value: 1_000_000, cost: 900_000, filled: 0, priced: 0 },
      온전("2026-08-24", 1_100_000),
    ]);
  ...
  it("온전한 날이 하나도 없으면 비교를 포기하지 않는다", () => {
    /* 기준이 아예 없으면 그래프가 통째로 사라진다.
       조금 어긋난 기준이라도 있는 편이 낫다 */
    const r = 견주기([
      반쪽("2026-08-20", 750_000),
      반쪽("2026-08-24", 800_000),
    ]);
    expect(r.기준칸).toBe(0);
    expect(r.점들[1].내수익).not.toBeNull();
  });
});
