...
describe("입금일을 같이 적는다", () => {
  ...
  it("공시된 지급일이 있으면 화면에 적는다", async () => {
    ...
    그리기();
    /* **줄에 적힌 입금일**을 본다(예: "입금 9/26").

       예전에는 /입금/ 로만 찾았다. 그런데 화면은 왼쪽 칸에도 남은 날을
       적는데, 기준일이 지나고 입금일이 아직이면 그 글이 "입금 D-10" 이다.
       그래서 달이 흘러 오늘이 그 사이에 들어오는 날 아침, 코드는 그대로인데
       '같은 글이 둘' 이라며 검사만 깨졌다. 실제로 그렇게 깨졌다.
       날짜가 붙은 쪽만 집어서 시간에 안 흔들리게 한다. */
    expect(await screen.findByText(/입금 \d+\/\d+/)).toBeInTheDocument();
  });
  ...
  it("기준일과 같으면 두 번 안 적는다", async () => {
    ...
    그리기();
    await screen.findByText("올해 받은 배당");
    /* 기준일과 같은 날이면 줄에 입금일을 따로 안 적는다.
       여기도 날짜가 붙은 쪽만 본다 — 왼쪽 칸의 "입금 D-n" 은 다른 것이고,
       그것까지 걸면 오늘이 며칠이냐에 따라 검사가 흔들린다. */
    expect(screen.queryByText(/입금 \d+\/\d+/)).toBeNull();
  });
});
