    ...
    portfolios.forEach(pf => {
      pf.items.forEach(item => {
        ...
        if (item.currentValueKRW != null) {
          ...
          // inputExchangeRate: 매수 당시 환율(항상 실수), exchangeRate: 실시간(0이면 미로드)
          const fx = item.currency === "USD"
            ? (item.inputExchangeRate ?? (exchangeRate > 0 ? exchangeRate : 1))
            : isUSDStock ? (exchangeRate > 0 ? exchangeRate : 1) : 1;
