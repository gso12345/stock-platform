...
export function 기준글(r: 배당줄, 달: number, 오늘 = new Date()): string | null {
  ...
  /* "작년이랑 올해랑 다를 텐데" — 맞는 말이다. 그래서 서버가 올해
     이미 받은 달들로 흐름을 재서 안 온 달에 얹는다. 얹었으면 그
     사실을 적는다. 근거를 안 밝히면 '작년 기준' 이라고 해 놓고 작년과
     다른 숫자를 보여 주는 셈이라 더 헷갈린다. */
  const 조정 = 칸.조정;
  if (조정 != null && Math.abs(조정 - 1) >= 0.005) {
    const 퍼센트 = Math.round((조정 - 1) * 100);
    return `${칸.year}년 기준 · 올해 ${퍼센트 >= 0 ? "+" : ""}${퍼센트}% 반영`;
  }
  return `${칸.year}년 기준`;
}
...
/* 짧은돈 은 utils/formatters 로 옮겼다 — 백테스트 결과 화면도 같은
   규칙이 필요한데, 여기서 가져가면 그 화면에 배당 달력 묶음이 통째로
   딸려 온다(같은 이유로 읽을수있는오류 도 옮긴 적이 있다).
   이 파일의 검사가 짧은돈 을 여기서 가져오므로 그대로 다시 내보낸다. */
