...
export function 견주기(점들: 자산흐름점[], 지수모음: 견줄지수[] = []): 견준결과 {
   ...
   * 기준일은 **온전한 날**에서 고른다.
   *
   * 예전에는 `value > 0` 만 봤다. 그런데 서버 기록에는 시세를 일부만
   * 받은 날이 섞일 수 있다 — 못 받은 종목을 매입가로 세므로 그날 값이
   * 실제보다 낮다(실측 25%까지). 그 날이 기준이 되면 **그 뒤 모든
   * 퍼센트가 부풀려진다**. 그리고 서버가 그 줄을 나중에 고쳐 쓰면
   * 같은 날의 수익률이 볼 때마다 달라진다.
   *
   * priced 는 '시세가 있어야 하는 종목 수', filled 는 '실제로 받은 수' 다.
   * 둘이 같아야 온전하다. 옛 기록에는 이 칸이 없을 수 있어(둘 다 0),
   * 그때는 예전처럼 value 만 보고 넘어간다 — 있는 정보로만 판단한다.
   */
  const 온전한가 = (p: 자산흐름점) => {
    const 쟀어야 = p.priced ?? 0;
    const 받았다 = p.filled ?? 0;
    if (쟀어야 <= 0) return true;          // 옛 기록이거나 현금뿐 — 판단할 근거가 없다
    return 받았다 >= 쟀어야;
  };
  const 온전한칸 = 점들.findIndex(
    (p) => p.value > 0 && 온전한가(p) && 쓸것.every((x) => x.그날종가(p.day)),
  );
  /* 온전한 날이 하나도 없으면 예전처럼 고른다. 기준이 아예 없으면
     비교가 통째로 사라지는데, 그건 조금 어긋난 기준보다 나쁘다 */
  const 기준칸 = 온전한칸 >= 0 ? 온전한칸 : 점들.findIndex(
