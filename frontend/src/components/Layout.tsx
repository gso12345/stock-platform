...
import { LayoutDashboard, Search, LineChart, Star, BookMarked, Activity, Sun, Moon } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-bg-base overflow-hidden">
        ...
        <div className="px-5 py-4 border-t border-border-subtle">
          <div className="flex items-center gap-2">
            <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
              ...
              wsStatus === "connecting" ? "bg-accent-yellow animate-pulse-slow" :
              "bg-text-dim"
            }`} />
            <span className="text-2xs text-text-dim">
              {wsStatus === "connected"  ? "실시간 연결됨" :
               wsStatus === "connecting" ? "연결 중..." : "오프라인"}
            </span>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* 헤더 */}
        <header
          className="h-13 flex-shrink-0 flex items-center px-6 gap-4"
          style={{
            background: "#0f1117",
        ...
        >
          ...
          <div className="flex items-center gap-3 text-2xs text-text-dim">
