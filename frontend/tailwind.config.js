  ...
  theme: {
    extend: {
      ...
      fontSize: {
        ...
        //
        // ── 아래 px 는 **root 16px 기준** 이다 ──
        //
        // 한동안 root 가 14px 이었다. rem 눈금이 전부 거기 곱해지니
        // 본문이 12.25px, 제일 작은 라벨이 9.6px 로 그려졌다 —
        // 브라우저 기본이 16px 인데 그보다 한참 작았다. '글씨가 작다'
        // 는 말이 나온 자리가 여기다.
        //
        // **눈금은 그대로 두고 root 만 16px 로 옮겼다**(index.css).
        // 눈금을 손으로 올리면 단계마다 다르게 커져 짜임새가 무너진다 —
        // 예전에 그렇게 했다가 되돌렸다. root 를 옮기면 모든 단계가
        // 같은 비율로 커져서 지금의 짜임새가 그대로 유지된다.
        //
        //   normal(16px)  large(18px)  xl(20px)  ← 설정의 글씨 크기
        //
        // ── 얼마나 키울 것인가 ────────────────────────────
        //
        ...
        "2xs": ["0.6875rem", "0.9375rem"],  //  11.0 / 12.4 / 13.8 px  가장 작은 라벨
        xs:    ["0.75rem", "1.0625rem"],  //  12.0 / 13.5 / 15.0 px  부가정보
        sm:    ["0.8125rem", "1.1875rem"],  //  13.0 / 14.6 / 16.2 px
        base:  ["0.875rem", "1.3125rem"],  //  14.0 / 15.8 / 17.5 px  본문·종목명
        md:    ["0.9375rem", "1.4375rem"],  //  15.0 / 16.9 / 18.8 px
        lg:    ["1rem", "1.5rem"],  //  16.0 / 18.0 / 20.0 px
        xl:    ["1.125rem", "1.75rem"],  //  18.0 / 20.2 / 22.5 px
        "2xl": ["1.375rem", "2rem"],  //  22.0 / 24.8 / 27.5 px
        "3xl": ["1.75rem", "2.25rem"],  //  28.0 / 31.5 / 35.0 px
        ...
        display: ["1.5rem", "1.875rem"],  //  24.0 / 27.0 / 30.0 px  가격·수익률
        hero:  ["1.875rem", "2.25rem"],  //  30.0 / 33.8 / 37.5 px  총자산·지수
      },
      borderRadius: {
