...
def 자산흐름(
    ...
    if not points or points[-1]["day"] != 날:
        ...
        if 환율 > 0 and 내것:
            ...
            예전에는 '하나도 못 받았을 때' 만 걸렀다. 그래서 세 종목 중
            하나만 받은 순간에 그래프 오른쪽 끝이 뚝 떨어졌다가, 잠시 뒤
            다시 물어보면 제자리로 돌아왔다 — 같은 날의 수익률이 볼
            때마다 달라 보이는 이유가 이것이다.

            못 받았으면 오늘 점을 아예 안 붙인다. 그래프가 어제에서
            끝나는 것이 틀린 값을 보여 주는 것보다 낫다."""
