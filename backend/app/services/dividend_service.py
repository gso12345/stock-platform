...
def _월별일정(최근: list, 날들: list, 주기: "str | None",
              증감: "float | None" = None) -> list:
    ...
    for m in sorted(칸):
        ...
        받은합 = sum(a for _, a in 같은해)

        """작년 기준 칸에는 **올해 흐름**을 반영한다.

        "작년이랑 올해랑 다를 텐데" — 맞는 말이다. 배당은 대개 해마다
        달라진다. 아직 안 온 달을 작년 금액 그대로 쓰면 그만큼 어긋난
        예상을 내놓는 셈이다.

        올해 이미 받은 달들이 작년 같은 달보다 10% 늘었으면, 안 온
        달에도 10% 를 얹는다. 근거가 '작년 금액' 하나뿐이던 것에서
        '작년 금액 × 올해 흐름' 으로 한 걸음 나아간 것이다.

        올해 확정된 달은 손대지 않는다 — 그건 추정이 아니라 사실이다.
        비율을 못 구했으면(겹치는 달이 적거나 값이 튀면) 그대로 둔다."""
        조정 = 1.0
        if 최신해 != 올해 and 증감:
            조정 = 증감
        결과.append({
            "month": m,
            "day": 같은해[-1][0].day,
            "amount": round(받은합 * 조정, 6),
            "year": 최신해,
            #: 작년 금액에 곱한 올해 흐름. 1.0 이면 안 건드린 것이다.
            #  화면이 '2025년 기준 · 올해 +10% 반영' 처럼 밝힐 수 있게 싣는다
            "조정": round(조정, 4),
            ...
            #: 날짜별 금액도 **같은 배수**로 맞춘다. 안 그러면 날짜별
            #  화면의 합이 위 amount 와 어긋나 두 화면이 다른 말을 한다
            "days": [{"date": d.isoformat(), "amount": round(a * 조정, 6)}
                     for d, a in 같은해],
...
#: 올해 흐름을 반영할 때 허용하는 배수 범위.
#
#  겹치는 달이 적으면 비율이 크게 흔들린다. 특별배당이 한 번 끼면
#  3배가 나오기도 한다 — 그 배수를 안 온 달에 그대로 곱하면 한 해
#  예상이 통째로 부풀어, 그 숫자로 생활비 계획을 세우는 사람이 생긴다.
증감_하한, 증감_상한 = 0.5, 2.0

#: 비율을 낼 때 최소 몇 달이 겹쳐야 하나. 한 달만 겹치면 그 달의
#  특성(결산배당 등)이 곧 비율이 된다.
증감_최소달 = 2


def 올해증감(최근: list, 날들: list, 오늘: "date | None" = None) -> "float | None":
    """올해 배당이 작년보다 얼마나 늘었나 — **겹치는 달만** 견준다.

    ── 왜 필요한가 ──

    아직 안 온 달은 근거가 작년 같은 달뿐이다. 그런데 배당은 대개
    해마다 달라진다 — 늘리는 회사가 많고, 줄이는 회사도 있다. 작년
    값을 그대로 쓰면 그만큼 어긋난 예상을 내놓는 셈이다.

    올해 이미 받은 달들을 작년 같은 달과 견주면 그 회사가 올해 어느
    쪽으로 가고 있는지가 나온다. 그 비율을 안 온 달에 적용한다.

    ── 왜 '겹치는 달만' 인가 ──

    올해는 아직 여덟 달치, 작년은 열두 달치다. 그냥 합을 나누면
    8/12 이 되어 '33% 삭감' 이라는 엉뚱한 답이 나온다. 두 해에 **다
    있는 달**만 골라 견줘야 한다.

    표본이 적으면 안 쓴다(None). 지어낸 정밀함보다 '작년 기준' 이라고
    솔직히 말하는 편이 낫다.
    """
    오늘 = 오늘 or date.today()
    올 = 오늘.year
    ...
    작년달: dict = {}
    for x, d in zip(최근, 날들):
        금액 = float(x.get("amount") or 0)
        if d.year == 올:
            올해달[d.month] = 올해달.get(d.month, 0.0) + 금액
        elif d.year == 작:
            작년달[d.month] = 작년달.get(d.month, 0.0) + 금액

    겹침 = sorted(set(올해달) & set(작년달))
    if len(겹침) < 증감_최소달:
        return None
    작년합 = sum(작년달[m] for m in 겹침)
    올해합 = sum(올해달[m] for m in 겹침)
    if 작년합 <= 0:
        return None
    비율 = 올해합 / 작년합
    if not (증감_하한 <= 비율 <= 증감_상한):
        # 특별배당 같은 것이 낀 값이다. 안 쓰는 편이 낫다
        return None
    return 비율
