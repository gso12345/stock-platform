...
def _환율_채워두기() -> None:
    ...
    여기서 기다리면 안 된다. 이 함수는 _do_fetch_kr_rates() 안에서
    불리는데, 그 함수 전체를 라우트가 **5초**에서 끊는다. 재 보니
    실패할 때 32.8초였다 — 여기서 왕복을 하나 더 얹으면 그만큼 더
    자주 끊기고, 끊기면 금리 목록이 통째로 빈 채로 화면에 나간다.

    받아 두면 다음 요청이 쓴다. 이번 요청은 이미 있는 것만 쓴다.
    """
    ...
    try:
        ...
        log.debug("환율 묶음 배경 맡기기 실패: %s", type(e).__name__)
...
def _do_fetch_kr_rates() -> list:
    ...
    덜 채워진 값을 완성품처럼 얼려 두면, 그 사이 원천이 살아나도 5분
    내내 옛 목록을 준다. 자산 스냅샷에서 고친 것과 같은 결함이다.

    무엇을 '반쪽' 으로 볼지는 이 목록이 **책임지는 것**으로 정한다.
    한때 여기서 환율(원/유로·원/100엔)이 있나를 봤는데, 그건 이제 이
    목록의 몫이 아니다 — 라우트가 캐시에서 따로 붙인다(kr_extras).
    금리 원천이 32초씩 걸려 목록이 통째로 비어도 환율은 뜨게 하려고
    그렇게 갈랐다.

    그래서 여기서는 **원천에서 실제로 받은 것이 있나**만 본다.

    '금리가 하나라도 있나' 로는 안 된다 — 아무 데서도 못 받으면 위에서
    정적 기본값(기준금리 2.75%)을 하나 세우기 때문에 그 조건이 늘 참이
    된다. 검사로 잡았다. 정적 표시(_static)가 붙은 것을 빼고 센다."""
    받아온것 = [x for x in rates if x.get("is_rate") and not x.get("_static")]
    온전한가 = bool(받아온것)
