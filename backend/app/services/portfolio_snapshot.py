...
def 찍기() -> int:
    ...
    try:
        ...
        for uid, 내것 in 묶음.items():
            ...
            바로 위 설명이 `filled < priced 인 줄은 '아직 덜 됐다' 로
            둔다` 고 적고 있는데, 검사는 `filled == 0`(하나도 못 받음)만
            걸러 왔다. 그래서 세 종목 중 하나만 받아도 그 줄이 그대로
            적혔다 — 못 받은 종목은 매입가로 세므로 그날이 통째로
            아래로 찍힌다(실측: 둘 중 하나만 받으면 **25% 아래**).

            빠진 날은 나중에 메우기() 가 그날 실제 종가로 채운다.
            반쪽 줄은 '구멍' 이 아니라서 메우기가 안 건드린다 —
            없는 것보다 **나쁘다.** 그래서 안 적는 쪽이 맞다."""
            if 합["priced"] > 0 and 합["filled"] < 합["priced"]:
            ...
            for pid, 그것들 in 나눠.items():
                쪽 = 합계내기(그것들, 환율)
                if 쪽["priced"] > 0 and 쪽["filled"] < 쪽["priced"]:
                    continue                 # 위와 같은 이유
                줄들.append((pid, 쪽))
    ...
    try:
        ...
        for uid, 내것 in 사람별.items():
            ...
            for pid, 그것들 in 나눔.items():
                ...
                시세를 일부만 받은 채 적힌 줄이다(filled < priced).
                못 받은 종목은 매입가로 세므로 그날이 통째로 아래로
                찍힌다 — 실측으로 두 종목 중 하나만 받으면 **25% 아래**다.

                찍기() 가 이제 그런 줄을 안 적지만, **이미 쌓인 것은
                그대로 남는다.** 그리고 반쪽 줄은 '구멍' 이 아니라서
                메우기가 건드리지도 않았다 — 앱을 안 연 날의 수익률이
                볼 때마다 달라 보이던 이유가 이것이다.

                여기서 지우면 그 자리가 구멍이 되고, 아래 메우기가 그날
                **실제 종가**로 다시 채운다. 없는 것보다 나쁜 값을
                남겨 두지 않는다."""
                def _반쪽인가(r) -> bool:
                    쟀어야 = r.priced or 0
                    받았다 = r.filled or 0
                    return 쟀어야 > 0 and 받았다 < 쟀어야

                모두 = 칸별.get((uid, pid)) or []
                줄들, 지울것 = [], []
                for r in 모두:
                    (지울것 if (_내가_메운것(r) or _반쪽인가(r)) else 줄들).append(r)
                for r in 지울것:
                    db.delete(r)
