...
NAVER_INDEX_CODES = {
    ...
    # ── 후보 (안 되면 스스로 물러난다) ──
    #
    # KRX300 을 뺐다. 코드 후보 셋을 다 걸어도 값이 안 왔고, 그 상태로
    # 화면에 자리를 차지했다 — 코스닥150 과 똑같은 일이 되풀이됐다.
    #
    # 여기 넣는 것은 '되는 걸 확인한 것' 만으로 한다. 확인 못 한 후보를
    # 넣어 두면 백오프가 요청은 막아 주지만, 그 지수가 화면 목록에는
    # 계속 남아서 "왜 안 나오냐" 를 다시 듣게 된다. 화면이 서버가 준
    # 것만 그리게 바꿨으므로(아래 fetch_naver_indices 참고) 여기서
    # 빠지면 화면에서도 저절로 빠진다.
