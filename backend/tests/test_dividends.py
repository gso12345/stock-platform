...
class Test응답을_가볍게:
    ...
    def test_해달별_합계를_같이_준다(self, 야후):
        ...
        묶음 = r["월별지난"]
        assert 묶음, "월별지난이 비었다"
        """한 달에 한 줄인가 — 그게 묶는다는 뜻이다.

        예전에는 '최대 24줄' 로 적었다. 틀린 어림이었다. 104주는
        약 23.9개월이지만 **달력 위에서는** 첫 지급일이 1일이 아닌 한
        25개월에 걸친다(2024-09-12 … 2026-09-03 이면 25달이다).
        그래서 오늘이 달의 어디쯤이냐에 따라 통과했다 안 했다 했다 —
        코드는 멀쩡한데 검사가 날짜를 타고 있었다.

        세어야 할 것은 줄 수가 아니라 '달이 겹치지 않는가' 다."""
        달들 = [(x["year"], x["month"]) for x in 묶음]
        assert len(달들) == len(set(달들)), f"같은 달이 두 줄이다: {달들}"
        assert len(묶음) < 104, f"안 묶였다: {len(묶음)}줄"
        """한 달 안의 회차를 합쳤는지 — 주배당은 달마다 4~5회다.

        '이번 달' 로 보면 안 된다. 달 초에 돌리면 이번 달 지급이 한
        번뿐이라 합계가 한 회차와 같고, 그러면 안 합쳤을 때와 구분이
        안 된다. 실제로 9월 3일에 그렇게 걸렸다.

        양 끝 달은 잘려 있으므로 가운데 달을 본다."""
        가운데 = 묶음[len(묶음) // 2]
        assert 가운데["amount"] > 0.063 * 3, (
            f"한 달치를 안 합쳤다: {가운데}")
...
class Test올해증감:
    """'배당 예상 기준이 뭐냐, 작년이랑 올해랑 다를 텐데' — 맞는 말이다.

    아직 안 온 달은 근거가 작년 같은 달뿐이다. 그런데 배당은 대개
    해마다 달라진다. 작년 값을 그대로 쓰면 그만큼 어긋난 예상을
    내놓는 셈이다.

    올해 이미 받은 달들을 작년 같은 달과 견주면 그 회사가 올해 어느
    쪽으로 가는지가 나온다. 그 비율을 안 온 달에 적용한다.
    """
    올해 = date.today().year

    def _내역(self, 쌍들):
        """[(연, 월, 금액), ...] → (최근, 날들)"""
        최근 = [{"date": f"{y}-{m:02d}-15", "amount": a} for y, m, a in 쌍들]
        날들 = [date(y, m, 15) for y, m, _ in 쌍들]
        return 최근, 날들

    def test_겹치는_달만_견준다(self):
        """올해는 여덟 달치, 작년은 열두 달치다. 그냥 합을 나누면
        8/12 이 되어 '33% 삭감' 이라는 엉뚱한 답이 나온다."""
        작 = self.올해 - 1
        최근, 날들 = self._내역([
            (작, 3, 100), (작, 6, 100), (작, 9, 100), (작, 12, 100),   # 작년 4회
            (self.올해, 3, 110), (self.올해, 6, 110),                   # 올해 2회
        ])
        비율 = DV.올해증감(최근, 날들, date(self.올해, 8, 1))
        assert 비율 == pytest.approx(1.1)      # 220/200 — 9·12월은 안 센다

    def test_줄어든_것도_잡는다(self):
        작 = self.올해 - 1
        최근, 날들 = self._내역([
            (작, 3, 100), (작, 6, 100),
            (self.올해, 3, 80), (self.올해, 6, 80),
        ])
        assert DV.올해증감(최근, 날들, date(self.올해, 8, 1)) == pytest.approx(0.8)

    def test_겹치는_달이_적으면_안_쓴다(self):
        """한 달만 겹치면 그 달의 특성(결산배당 등)이 곧 비율이 된다.
        지어낸 정밀함보다 '작년 기준' 이라고 솔직히 말하는 편이 낫다."""
        작 = self.올해 - 1
        최근, 날들 = self._내역([(작, 3, 100), (self.올해, 3, 200)])
        assert DV.올해증감(최근, 날들, date(self.올해, 8, 1)) is None

    def test_말도_안_되게_튀면_안_쓴다(self):
        """특별배당이 한 번 끼면 3배가 나오기도 한다. 그 배수를 안 온
        달에 곱하면 한 해 예상이 통째로 부풀어, 그 숫자로 생활비
        계획을 세우는 사람이 생긴다."""
        작 = self.올해 - 1
        최근, 날들 = self._내역([
            (작, 3, 100), (작, 6, 100),
            (self.올해, 3, 500), (self.올해, 6, 100),      # 3배
        ])
        assert DV.올해증감(최근, 날들, date(self.올해, 8, 1)) is None

    def test_올해_기록이_없으면_안_쓴다(self):
        작 = self.올해 - 1
        최근, 날들 = self._내역([(작, 3, 100), (작, 6, 100)])
        assert DV.올해증감(최근, 날들, date(self.올해, 8, 1)) is None


class Test예상에_올해_흐름을_반영한다:
    올해 = date.today().year
    ...
    def test_안_온_달에_올해_흐름을_얹는다(self):
        """작년 9월에 100 을 줬고 올해가 10% 늘고 있으면, 9월 예상은
        110 이다 — 작년 그대로인 100 이 아니라."""
        작 = self.올해 - 1
        최근, 날들 = self._줄([
            (작, 3, 100), (작, 6, 100), (작, 9, 100),
            (self.올해, 3, 110), (self.올해, 6, 110),
        ])
        증감 = DV.올해증감(최근, 날들, date(self.올해, 8, 1))
        일정 = DV._월별일정(최근, 날들, "분기", 증감)
        구월 = next(x for x in 일정 if x["month"] == 9)
        assert 구월["year"] == 작                       # 근거는 작년 것
        assert 구월["amount"] == pytest.approx(110)     # 값은 올해 흐름 반영
        assert 구월["조정"] == pytest.approx(1.1)       # 화면이 밝힐 수 있게

    def test_올해_확정된_달은_안_건드린다(self):
        """그건 추정이 아니라 사실이다."""
        작 = self.올해 - 1
        최근, 날들 = self._줄([
            (작, 3, 100), (작, 6, 100),
        ...
        증감 = DV.올해증감(최근, 날들, date(self.올해, 8, 1))
        일정 = DV._월별일정(최근, 날들, "분기", 증감)
        삼월 = next(x for x in 일정 if x["month"] == 3)
        assert 삼월["amount"] == pytest.approx(110)     # 실제 받은 값 그대로
        assert 삼월["조정"] == pytest.approx(1.0)

    def test_비율을_못_구하면_작년_그대로_둔다(self):
        """지어낸 정밀함보다 솔직한 편이 낫다."""
        작 = self.올해 - 1
        최근, 날들 = self._줄([(작, 3, 100), (작, 9, 100)])
        일정 = DV._월별일정(최근, 날들, "분기", DV.올해증감(최근, 날들, date(self.올해, 8, 1)))
        구월 = next(x for x in 일정 if x["month"] == 9)
        assert 구월["amount"] == pytest.approx(100)
        assert 구월["조정"] == pytest.approx(1.0)

    def test_날짜별_금액도_같은_배수로_맞춘다(self):
        """날짜별 화면의 합이 위 amount 와 어긋나면 두 화면이 다른
        말을 한다."""
        작 = self.올해 - 1
        최근, 날들 = self._줄([
            (작, 3, 100), (작, 6, 100), (작, 9, 100),
        ...
        일정 = DV._월별일정(최근, 날들, "분기", 증감)
        구월 = next(x for x in 일정 if x["month"] == 9)
        assert sum(d["amount"] for d in 구월["days"]) == pytest.approx(구월["amount"])
