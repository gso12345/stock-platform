...
class Test거래비용:
    """수수료·세금은 작아 보여도 30년이면 몇 %를 갉는다.

    그리고 이 자리는 **틀려도 티가 안 난다.** 비용을 절반만 매겨도
    결과는 그럴듯하고, 오히려 성적이 좋아 보이니 아무도 의심하지 않는다.
    """

    def test_손으로_센_값과_같다(self):
        """살 때 한 번. 1,000만원어치를 0.1% 로 사면 1만원이다.
        리밸런싱이 없으니 그 뒤로는 안 나간다."""
        날들 = 날들만들기(300)
        r = P.돌리기({"A": 표(날들, [100] * 300)},
                     [{"symbol": "A", "weight": 1}],
                     10_000_000, 거래비용=0.001)
        assert r["costs"] == pytest.approx(10_000)
        assert r["final_value"] == pytest.approx(9_990_000, rel=1e-6)

    def test_비용만큼_덜_산다(self):
        """수수료를 '나중에 빼는 돈' 으로 두면 안 된다. 실제로는 살 때
        그만큼 못 사는 것이고, 못 산 몫은 그 뒤로 못 굴린다 —
        차이가 복리로 벌어진다."""
        날들 = 날들만들기(760)
        값 = [100 * (1.002 ** i) for i in range(760)]      # 크게 오른다
        자산 = [{"symbol": "A", "weight": 1}]
        없이 = P.돌리기({"A": 표(날들, 값)}, 자산, 10_000_000)
        같이 = P.돌리기({"A": 표(날들, 값)}, 자산, 10_000_000, 거래비용=0.001)
        차이 = 없이["final_value"] - 같이["final_value"]
        assert 차이 > 10_000 * 2, \
            (f"차이가 {차이:,.0f} 뿐이다 — 낸 수수료(1만원)만큼만 줄었다면 "
             "비용을 나중에 빼고 있는 것이다. 못 산 몫도 같이 굴었어야 한다")

    def test_현금에는_안_붙는다(self):
        """현금은 사고파는 것이 아니다. 100% 현금이면 수수료가 0 이다."""
        날들 = 날들만들기(300)
        r = P.돌리기({"A": 표(날들, [100] * 300)},
                     [{"symbol": "A", "weight": 50}, {"symbol": "현금", "weight": 50}],
                     10_000_000, 거래비용=0.001)
        assert r["costs"] == pytest.approx(5_000), \
            "현금 몫에도 수수료를 매겼다"

    def test_리밸런싱할_때도_현금은_빼고_센다(self):
        """위 검사는 **첫날**만 본다. 수수료를 세는 자리는 두 곳이고,
        리밸런싱 쪽에도 같은 규칙이 따로 있다.

        그쪽을 빠뜨리면 현금에는 시세표가 없으므로 값을 찾다가
        KeyError 로 터진다 — 즉 현금을 담고 리밸런싱을 켠 사람만
        500 을 본다. (첫날만 검사했더니 뮤테이션이 그대로 살아남았다.)

        손으로 센다 — A 가 두 배, 50/50 에 현금 절반, 1,000만원, 1%.
          첫날    A 몫 500만 중 5만이 수수료(현금 500만에는 안 붙는다)
          A 두 배 A 990만 · 현금 500만 → 합 1,490만
          목표    A 745만 · 현금 745만
          거래    A 를 245만 판다 → A 한 쪽만 센다
          수수료  245만 × 1% = 24,500원
        """
        날들 = 날들만들기(300)
        중간 = 150
        A = [100.0] * 중간 + [200.0] * (300 - 중간)
        r = P.돌리기({"A": 표(날들, A)},
                     [{"symbol": "A", "weight": 50}, {"symbol": "현금", "weight": 50}],
                     10_000_000, 리밸런싱="yearly", 거래비용=0.01)
        리밸비용 = r["costs"] - 5_000_000 * 0.01
        assert 리밸비용 == pytest.approx(24_500, rel=0.02), \
            f"리밸런싱 수수료가 {리밸비용:,.0f} 이다 — 현금까지 거래액에 넣었다"

    def test_리밸런싱은_판_쪽과_산_쪽_양쪽에_붙는다(self):
        """100을 팔아 100을 사면 200어치를 거래한 것이다.

        한쪽만 매기면 비용이 절반이라, **자주 리밸런싱할수록 유리해
        보이는** 거꾸로 된 결과가 나온다.

        손으로 세 본다 — A 가 두 배, B 는 그대로, 60/40 으로 시작해
        1,000만원, 수수료 1%.

          첫날    1,000만 중 1%(10만)가 수수료로 나가고
                  A 594만 · B 396만이 실제로 담긴다
          A 두 배 A 1,188만 · B 396만 → 합 1,584만
          목표    A 950.4만 · B 633.6만
          거래    A 를 237.6만 팔고 B 를 237.6만 산다 → 합 475.2만
          수수료  475.2만 × 1% = 47,520원

        (처음에는 첫날 수수료를 잊고 9,600원으로 적었다가 걸렸다.
         비용을 뺀 만큼 덜 샀다는 것이 여기까지 따라온다.)"""
        날들 = 날들만들기(300)
        중간 = 150
        A = [100.0] * 중간 + [200.0] * (300 - 중간)     # 딱 한 번 두 배
        r = P.돌리기({"A": 표(날들, A), "B": 표(날들, [100.0] * 300)},
                     [{"symbol": "A", "weight": 60}, {"symbol": "B", "weight": 40}],
                     10_000_000, 리밸런싱="yearly", 거래비용=0.01)
        첫날비용 = 10_000_000 * 0.01
        리밸비용 = r["costs"] - 첫날비용
        assert 리밸비용 == pytest.approx(47_520, rel=0.001), \
            (f"리밸런싱 비용이 {리밸비용:,.0f} 이다. 23,760 쯤이면 "
             "한쪽에만 매기고 있는 것이다")

    def test_비용률에_비례한다(self):
        날들 = 날들만들기(760)
        가격 = {"A": 표(날들, [100 * (1.001 ** i) for i in range(760)]),
                "B": 표(날들, [100] * 760)}
        자산 = [{"symbol": "A", "weight": 50}, {"symbol": "B", "weight": 50}]
        싼것 = P.돌리기(가격, 자산, 10_000_000, 리밸런싱="monthly", 거래비용=0.001)
        비싼것 = P.돌리기(가격, 자산, 10_000_000, 리밸런싱="monthly", 거래비용=0.005)
        assert 비싼것["costs"] > 싼것["costs"] * 4
        assert 비싼것["final_value"] < 싼것["final_value"]

    def test_0_이면_안_넣은_것으로_적는다(self):
        """'안 넣었다' 와 '넣었는데 0원' 은 다른 말이다"""
        날들 = 날들만들기(300)
        r = P.돌리기({"A": 표(날들, [100] * 300)},
                     [{"symbol": "A", "weight": 1}], 10_000_000)
        assert r["costs"] is None and r["cost_rate"] is None


class Test리밸런싱_날짜:
    def test_기준일이_지난_첫_거래일을_고른다(self):
        """15일이 주말이면 16일이나 17일이다. 날짜를 딱 맞춰 찾으면
        그 달이 통째로 빠진다."""
        날들 = 날들만들기(500, date(2020, 1, 1))
        고른것 = sorted(P.주기날들(날들, "monthly", 15))
        assert all(d.day >= 15 for d in 고른것), \
            f"기준일 앞의 날을 골랐다 — {[d for d in 고른것 if d.day < 15][:3]}"
        assert all(d in 날들 for d in 고른것), "거래일이 아닌 날을 골랐다"

    def test_달마다_딱_한_번만_고른다(self):
        """이게 이 함수에서 제일 틀리기 쉬운 자리다. 조건만 보고 고르면
        15일부터 월말까지 **매일** 걸려서 적립이 열 배로 불어난다."""
        날들 = 날들만들기(500, date(2020, 1, 1))
        for 기준 in (1, 10, 15, 28):
            고른것 = P.주기날들(날들, "monthly", 기준)
            달들 = {(d.year, d.month) for d in 고른것}
            assert len(고른것) == len(달들), \
                f"기준일 {기준} 일 때 한 달에 두 번 이상 골랐다"

    def test_기준일을_바꾸면_다른_날이_나온다(self):
        날들 = 날들만들기(500, date(2020, 1, 1))
        assert P.주기날들(날들, "monthly", 1) != P.주기날들(날들, "monthly", 20)

    def test_적립도_기준일을_따른다(self):
        """리밸런싱만 고치고 적립을 안 고치면, 월급날에 맞춰 넣겠다고
        골라도 적립은 1일에 들어간다."""
        날들 = 날들만들기(500)
        r = P.돌리기({"A": 표(날들, [100] * 500)},
                     [{"symbol": "A", "weight": 1}],
                     1_000_000, 적립주기="monthly", 적립금액=100_000, 적립날짜=20)
        달수 = len(P.주기날들(날들, "monthly", 20))
        assert r["contributed"] == pytest.approx(1_000_000 + 달수 * 100_000)
